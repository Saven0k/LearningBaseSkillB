...
async function checkNetworkStatus() {
  try {
    ...
    const response = await fetch('https://sb-film.skillbox.cc/ping', { // пустышка
      method: 'POST',
    ...
    networkStatus.isSlow = responseTime > 500;
    networkStatus.isOnline = true;
  } catch (error) {
    networkStatus.isOnline = false;
    networkStatus.isSlow = false;
  }
  
  updateNetworkNotification();
}

function updateNetworkNotification() {
  ...
  if (!networkStatus.isOnline) {
    ...
    notification.className = 'network-notification error';
  } else if (networkStatus.isSlow) {
    notification.textContent = 'Медленное соединение';
    notification.style.display = 'block';
    notification.className = 'network-notification warning';
  } else {
    notification.style.display = 'none';
  }
}

// Запускаем проверку каждые 5 сек
